...
for m in range(12):
    ...
    for n in range(11):
        ...
        city = pow(city_fz / city_fm, 1/11)
        cat.append(city)
    pw.append(cat)
cat_p = np.array(pw)

Big_w = np.array(temp_BigW)
big_pw = cat_p * Big_w[:,1:]
...
for i in range(11):
    ...
    temp_bigfm = []
    for c in range(11):
        temp_bigfm1 = temp_p[:,i+1] * temp_w[:,c+1]
        temp_bigsum = np.sum(temp_bigfm1)
        temp_bigfm.append(temp_bigsum)
            
    big_city_fm = pow(sum_big_pw[i],11)
    big_city = pow(big_city_fz / big_city_fm, 1/11)
    big_p.append(big_city)
result = pd.DataFrame(np.array(big_p).reshape(1,-1))
